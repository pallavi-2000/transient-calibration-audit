...
Our initial collection did not record the \texttt{classifier\_version}
tag returned alongside each probability vector; we later re-queried the
API specifically to recover it (Section~\ref{sec:results:version}) and
found two distinct versions active across our collection window,
\texttt{hierarchical\_rf\_1.1.0} and \texttt{lc\_classifier\_1.1.13}, for
1,601 of 1,606 objects (5 API failures on re-query). We provide the full
list of ZTF object identifiers, retrieved probability vectors, and
recovered version tags as a machine-readable supplement to this paper.
