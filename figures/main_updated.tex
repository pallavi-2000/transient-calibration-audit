...
\begin{abstract}
Production transient classifiers deployed on the Zwicky Transient Facility (ZTF) alert stream are routinely used to prioritise spectroscopic follow-up, yet to our knowledge, no prior work has systematically evaluated calibration across multiple production brokers using standard metrics on real survey data. We present a systematic calibration audit of four classifiers: two production alert brokers (ALeRCE's Balanced Random Forest, and Fink's Random Forest and SuperNNova) and the standalone NEEDLE classification framework, using spectroscopically confirmed transients from the ZTF Bright Transient Survey as ground truth. We compute Expected Calibration Error (ECE) with equal-mass binning, classwise reliability diagrams, and Brier scores, then apply temperature scaling with five-fold cross-validation. We find that all four classifiers exhibit significant miscalibration. ALeRCE is systematically underconfident (ECE $= 0.271$); temperature scaling ($T = 0.357$) reduces ECE to $0.097$ (65\% improvement), increasing objects exceeding $p > 0.8$ from 20 to 425 at 91\% precision. Fink's Random Forest, designed as an early-time SN~Ia classifier \citep{leoni2022}, abstains on 93.9\% of BTS objects (1,161 of 1,237) with near-uniform abstention rates across true classes (90--99\%); we identify this as operational regime mismatch rather than miscalibration and therefore exclude RF from probabilistic calibration analysis. Fink's SuperNNova classifier exhibits implicit selective classification, emitting non-zero scores for 64.1\% of BTS objects (793/1,237) with strongly class-dependent abstention ($\chi^2$, $p = 8 \times 10^{-37}$; non-Ia transients abstain at 2.6$\times$ the rate of SN~Ia). On its accepted set, conditional ECE is 0.183 [0.154, 0.220], reducible to 0.051 with temperature scaling $T = 3.65$. However, this calibration improvement is operationally null at high thresholds: zero objects pass $p \ge 0.8$ post-scaling (versus 209 pre-scaling at 73\% precision), because $T > 1$ compresses overconfident scores rather than exposing additional candidates as ALeRCE's $T < 1$ sharpening does. NEEDLE achieves good aggregate calibration at the object level (ECE $= 0.048$, $N = 278$ unique objects), but per-class analysis reveals severe class-imbalance-driven miscalibration: SLSN-I (superluminous supernovae type I, $n = 15$ true objects) are predicted with 73\% mean confidence but only 31\% accuracy (gap $= -0.423$), with three objects confidently misclassified as SNe at $> 0.998$ confidence. Global temperature scaling worsens calibration, and per-class scaling cannot resolve the structural miscalibration. These findings characterize the calibration landscape of current ZTF-era broker classifiers and suggest implications for the Vera C.\ Rubin Observatory / LSST era, while we caution that direct transferability is bounded by differences in alert cadence, photometric depth, and broker pipeline evolution. Code and results are available at \url{https://github.com/pallavi-2000/transient-calibration-audit}.
\end{abstract}
...
Of the 1{,}436 BTS objects in the stratified evaluation sample, broker
predictions were retrieved for 1{,}149 (ALeRCE; 80.0\%) and 1{,}237
(Fink; 86.1\%). To verify that the missing objects do not introduce
systematic bias into the calibration audit, we tested whether retrieval
is class-dependent and whether retrieved and missing objects differ on
observable BTS properties. Table~\ref{tab:missingness} reports per-class
missing rates with 95\% Wilson confidence intervals and chi-squared tests
for class-dependence.

ALeRCE retrieval is significantly class-dependent ($\chi^2$
$p = 4.1 \times 10^{-4}$): SN~Ibc transients are most likely to be missed
(25.0\% missing rate; 95\% CI [0.20, 0.30]) while
TDE and SLSN have lower missing rates (10.3\% and 12.4\%, respectively).
A Mann-Whitney $U$ comparison of observable properties between retrieved
and missing ALeRCE objects further reveals significantly longer median
durations for retrieved objects (26.3 versus 24.4 days;
$p = 4.2 \times 10^{-5}$) and no significant differences in peak magnitude
or redshift. This is consistent with the ALeRCE light-curve classifier's
documented minimum-detection requirements \citep{sanchezsaez2021}: shorter,
less-evolved transients are more likely to fail eligibility criteria.
ALeRCE-side calibration metrics in this paper should therefore be
interpreted as conditional on the retrieval-eligible subset of BTS-bright
transients.

Fink retrieval shows no significant class-dependence ($\chi^2$ $p = 0.061$)
and no significant property differences, consistent with Fink's
classifier-agnostic acquisition pipeline that returns scores (including
zeros) for all submitted alerts. Note that Fink \emph{coverage} (the
fraction with non-zero scores) is strongly class-dependent and is
analyzed separately in Section~\ref{sec:fink_snn}; the missingness
analyzed here refers to \emph{retrieval}, i.e., whether any score was
returned at all.

\begin{table*}[t]
\centering
\caption{Class-dependent missingness analysis. Missing rate is the
fraction of BTS objects in each class for which no broker prediction
was retrieved. 95\% confidence intervals are Wilson binomial.
$\chi^2$ statistics test class-dependence of retrieval. ALeRCE missingness
is significantly class-dependent and biased toward shorter-duration
transients; Fink missingness is approximately uniform.
\label{tab:missingness}}
\begin{tabular}{lcccccc}
\toprule
 & \multicolumn{3}{c}{ALeRCE ($\chi^2$ $p = 4.1 \times 10^{-4}$)}
 & \multicolumn{3}{c}{Fink ($\chi^2$ $p = 0.061$)} \\
\cmidrule(lr){2-4}\cmidrule(lr){5-7}
Class & $n_{\rm BTS}$ & $n_{\rm missing}$ & Missing rate [95\% CI]
& $n_{\rm BTS}$ & $n_{\rm missing}$ & Missing rate [95\% CI] \\
\midrule
SN~Ia  & 600 & 137 & 0.228 [0.197, 0.264] & 600 & 73 & 0.122 [0.098, 0.150] \\
SN~Ibc & 300 &  75 & 0.250 [0.204, 0.302] & 300 & 55 & 0.183 [0.144, 0.231] \\
SN~II  & 400 &  59 & 0.147 [0.116, 0.186] & 400 & 55 & 0.138 [0.107, 0.175] \\
SLSN   &  97 &  12 & 0.124 [0.072, 0.204] &  97 & 14 & 0.144 [0.088, 0.228] \\
TDE    &  39 &   4 & 0.103 [0.041, 0.236] &  39 &  2 & 0.051 [0.014, 0.169] \\
\bottomrule
\end{tabular}
\end{table*}
...
Several terms recur throughout this audit that benefit from consolidated
definitions:

\textit{Coverage} is the fraction of evaluation-sample objects for which
a classifier emits a non-zero score. For ALeRCE, coverage is 100\%
(every retrieved object receives a probability vector). For Fink,
coverage is the fraction with score $> 0$, distinct from retrieval
(whether any score is returned by the API at all;
Section~\ref{sec:missingness}).

\textit{Accepted set} refers to the subset of objects for which a
classifier emits a non-zero (usable) prediction. For Fink classifiers
this is the subset with score $> 0$; the complement is the
\textit{abstention set}, where score $= 0$ indicates eligibility
failure.

\textit{Conditional calibration} is calibration error computed only on
the accepted set, distinct from \textit{unconditional calibration} which
includes abstentions. For Fink classifiers, only conditional calibration
is meaningful because abstentions do not represent valid probabilities
(Section~\ref{sec:fink}).

\textit{Abstention bias} is the difference between conditional ECE
computed under the accepted-set class composition and conditional ECE
reweighted to a target population prior. A positive value indicates
that the conditional measurement underestimates population-level
miscalibration because abstention preferentially removes harder cases
(Section~\ref{sec:fink_snn}).

\textit{Operationally null calibration} refers to a calibration
transformation that improves formal metrics (e.g., reduces ECE) but
does not change downstream operational decisions at practically-used
thresholds. We document this explicitly for Fink SuperNNova
(Section~\ref{sec:fink_snn}) and contrast it with ALeRCE's
operationally-effective calibration (Section~\ref{sec:alerce}).

\textit{Object-level} versus \textit{model-instance} analyses, used
specifically for NEEDLE, distinguish between treating each unique ZTF
object as one prediction (object-level, $N = 278$) versus treating each
(object, held-out-model) pair as an independent prediction
(model-instance, $N = 429$). Object-level is the primary analysis;
model-instance is a sensitivity check
(Section~\ref{sec:needle_sensitivity}).

\textit{Silent failure} refers to a high-confidence prediction
($\geq 90\%$) that is incorrect. Silent failures are particularly
concerning for spectroscopic triage because they are unlikely to be
...
For binary classifiers (Fink RF and SuperNNova), confidence is defined as $\max(p, 1-p)$ and predictions are thresholded at $p = 0.5$. We verified that ECE is robust to the choice of bin count for our primary NEEDLE sample (object-level, $N = 278$): ECE varies from 0.036 at $M = 5$--10 to 0.061 at $M = 20$; see Appendix~\ref{app:bins} for the full bin-sensitivity analysis.
...
\textbf{Application to Random Forest outputs.} Temperature scaling was
originally formulated for softmax-based neural network classifiers
\citep{guo2017}, which produce logits $z_k$ directly. Random Forest
classifiers (used by ALeRCE and Fink RF) instead produce vote-fraction
probabilities, which are not native logits. To apply temperature scaling
to RF outputs, we convert vote-fractions $p_k$ to pseudo-logits via
$z_k = \log p_k$ before applying temperature scaling. We emphasise that
this is a heuristic transformation rather than a theoretically-grounded
one: the assumption that RF vote-fraction distributions admit a
softmax-style temperature parameter is not justified by the underlying
ensemble-averaging mechanism. We adopt this transformation for the
practical reason that it is the most common workaround in the calibration
literature for non-logit-producing classifiers \citep{niculescumizil2005},
and we report results with explicit acknowledgment that the empirical
success of $T = 0.357$ for ALeRCE (Section~\ref{sec:alerce}) demonstrates
effectiveness in this setting without proving theoretical optimality.
Readers interpreting the ALeRCE temperature-scaling result should bear in
mind that alternative transformations (e.g., Platt scaling applied directly
to vote-fractions, or isotonic regression which is
fully non-parametric) may be equally or more appropriate; we discuss this
caveat further in Section~\ref{sec:limitations}.
...
Fink RF$^{\dagger}$ & RF & 2 & 1237 & --- & --- & N/A (regime mismatch) & --- \\
Fink SNN$^{\ddagger}$ (cond.) & RNN & 2 & 793 & $0.183$ $[0.154, 0.220]$ & 0.274 & Temp.\ ($T=3.65$) & $0.051$ \\
\hline
NEEDLE (obj.-level)$^{\ddagger}$ & CNN+DNN & 3 & 278 & $0.048$ $[0.048, 0.111]$ & 0.265 & Global $T$ worsens & $0.169$ \\
\quad (model-instance) & & & 429 & $0.073$ $[0.063, 0.117]$ & 0.241 & (sensitivity; \S\ref{sec:needle_sensitivity}) & --- \\
\enddata
\tablecomments{ECE computed with equal-mass binning (15 bins). 95\% confidence intervals via bootstrap (1,000 replicates). Post-hoc ECE for ALeRCE is the cross-validated estimate (stratified 5-fold). Brier score: 0 is perfect; for reference, random guessing gives $\sim$0.5 for binary classifiers and $\sim$1.5 for 4-class. $^{\dagger}$Fink RF abstains on 93.9\% of BTS objects with near-uniform abstention across classes (regime mismatch; see \S\ref{sec:fink_rf}); ECE on the 76 non-zero predictions is not population-representative and is omitted. $^{\ddagger}$Fink SNN conditional metrics on 793 non-zero predictions (64.1\% coverage). Reweighting to BTS prevalence yields population-equivalent gap of 0.460 (abstention bias $+0.024$). Post-$T$ value of 0.051 is operationally null at $p \ge 0.8$; see Table~\ref{tab:fink_snn_thresholds} and \S\ref{sec:fink_snn}. $^{\S}$NEEDLE object-level (primary): 278 unique objects, deduplicated by averaging probabilities across models. See \S\ref{sec:needle} and \S\ref{sec:robustness}.}
\end{deluxetable*}
...
The Fink broker's two probabilistic classifiers---Random Forest (\texttt{rf\_snia\_vs\_nonia}) and SuperNNova (\texttt{snn\_snia\_vs\_nonia})---both return score $= 0$ for objects that fail eligibility criteria (minimum-epoch requirements, host-galaxy cuts, light-curve length cuts; \citealt{leoni2022, moller2020}). Score $= 0$ does not represent $P(\text{SN~Ia}) = 0$; it represents classifier abstention. Treating zeros as ordinary probabilities yields meaningless ECE values and obscures the actual classifier behavior. We therefore reframe the Fink analysis as a \textbf{selective classification} problem \citep{geifman2017}: each classifier defines an acceptance gate $\pi(x) \in \{0, 1\}$ and a probability $p(y \mid x)$ on the accepted subset. We evaluate three properties separately: coverage (does the classifier emit a probability?), conditional calibration (are the emitted probabilities reliable?), and abstention bias (does the abstention pattern bias the conditional calibration estimate?).

Because BTS contains spectroscopically confirmed transients, every object in our sample is a real astrophysical event. Zero-score objects are therefore real transients on which the classifier did not emit a usable prediction, not non-transient distractors.
...
\label{sec:implications}The ALeRCE result generalizes only to underconfident classifiers. For overconfident classifiers, calibration produces the opposite operational consequence. Fink SuperNNova's pre-calibration score distribution spans [0.005, 0.962] with 209 BTS objects passing $p \ge 0.8$ at 73.2\% precision. Temperature scaling with $T = 3.65$ reduces conditional ECE by 72\% but compresses the score distribution to [0.191, 0.708], eliminating all $p \ge 0.8$ predictions (Table~\ref{tab:fink_snn_thresholds}). The ECE improvement is real in the calibration sense---a stated 0.6 post-$T$ score corresponds to 60\% empirical SN~Ia rate---but operationally it reflects the classifier honestly expressing its inherent uncertainty rather than gaining discriminative power. For high-precision triage, pre-calibration scores remain operationally useful despite their miscalibration. We therefore recommend that broker outputs be reported with both calibration metrics and threshold-curve diagnostics, because calibration alone does not determine operational utility for overconfident classifiers.

\subsection{Implications for the LSST Era and Transferability Caveats}
\label{sec:lsst_implications}

The Vera C.\ Rubin Observatory began issuing science alerts in early 2026,
and full survey operations will follow with an alert volume substantially
exceeding ZTF. Our audit findings on ZTF-era broker classifiers carry
implications for the Rubin/LSST era, but those implications are
extrapolations under several domain shifts that we make explicit here.

\textbf{Calibration-method recommendations transfer.} The general
finding that production transient classifiers exhibit heterogeneous
miscalibration patterns---underconfident for class-balanced ensemble
methods, overconfident for deep-learning binary classifiers, and
class-asymmetric for inverse-frequency-weighted multiclass
models---is rooted in classifier architecture and training procedure
rather than in survey specifics. We expect the same regime taxonomy to
apply to LSST-era broker classifiers, and the methodological
recommendations (equal-mass ECE binning, classwise reliability
evaluation, selective-classification framing for abstention-prone
classifiers, prior-aware operational metrics) transfer directly.

\textbf{Specific calibration values do not directly transfer.} The
quantitative miscalibration values we report (e.g., ALeRCE pre-scaling
ECE of 0.271, post-scaling 0.097) are conditional on (i) the ZTF alert
cadence and depth, (ii) the BTS-bright spectroscopic regime, and (iii)
the specific classifier versions audited. LSST cadence
\citep{ivezic2019}, photometric depth, and broker pipeline
evolution \citep{moller2021,sanchezsaez2021} differ in ways that will
likely produce different absolute calibration values. We do not claim
our values constitute Rubin-era forecasts.

\textbf{Operational threshold counts do not directly transfer.} Our
21$\times$ ALeRCE gain factor at $p \geq 0.8$ depends on the
alert-stream class prior and on the BTS-bright magnitude limit.
LSST will probe fainter, more numerous, and more diverse transients,
with different class priors and detection efficiencies.
Section~\ref{sec:prior_sensitivity} shows that the gain factor varies
between 21$\times$ and 33$\times$ across three plausible prior schemes;
the LSST-deployment value lies outside the bracket of priors we tested
and must be re-derived.

\textbf{What would constitute LSST-direct validation.} A future audit
aiming for direct LSST applicability would need: (a) an LSST alert-stream
sample with spectroscopic ground truth (currently limited; LSST DDF plus
targeted spectroscopic follow-up programs may provide this within a few
years), (b) audited classifier versions deployed on LSST alert packets
(which include richer DIA history than ZTF alerts), and (c)
cadence-aware sensitivity analyses across the LSST baseline survey
strategy. Until such validation exists, the present work should be
regarded as a ZTF-era audit whose findings inform but do not directly
verify LSST expectations.

\textbf{Concrete community recommendations.} Our findings motivate three
concrete actions for the Rubin-era broker community:
\begin{itemize}
  \item Broker teams should publish calibration metrics (ECE, classwise
        reliability, Brier score) alongside accuracy figures as
        standard reporting.
  \item Selective-classification metrics---coverage, conditional
        calibration, and abstention bias---should be standard for any
        classifier with eligibility gates.
  \item Operational threshold counts and gain factors should be reported
        for multiple plausible deployment-time class priors, not only
        for the audit-time evaluation sample.
\end{itemize}
...
\textbf{Fink RF calibration method comparison: not applicable.} We initially evaluated temperature scaling, Platt scaling, and isotonic regression on Fink RF's 76 non-zero predictions (5-fold CV) and observed nominal ECE values of 0.175 (T-scaling), 0.198 (Platt), and 0.123 (isotonic). However, because the 76 non-zero objects are an idiosyncratic subset of BTS that happened to pass the early-classifier eligibility gate (Section~\ref{sec:fink_rf}), this comparison does not reflect calibration performance on the intended Fink RF target population. We therefore omit Fink RF calibration-method recommendations from this paper and recommend that future calibration audits of the early-classifier evaluate on alert-stream samples within its design regime. Figure~\ref{fig:fink_methods} is retained in Appendix~\ref{app:methods} for transparency.
...
Operational threshold metrics (number of objects passing $p \geq 0.8$
and the resulting precision) depend not only on classifier calibration
but also on the class composition of the evaluation sample. Our headline
ALeRCE result---a 21.2$\times$ expansion of the high-confidence
selection set---is computed on the stratified evaluation sample
designed for per-class statistical power, where SN~Ia comprises
41.6\% of the audited objects. To assess whether this gain claim
generalises to deployment-realistic class priors, we recompute the
threshold counts under two alternative weighting schemes
(Table~\ref{tab:prior_sensitivity}, Figure~\ref{fig:prior_sensitivity}):

\begin{enumerate}
\item \textbf{Natural BTS prior}: per-class weights derived from the
pre-stratification BTS objects within the four transient classes,
in which SN~Ia comprises 42.9\% (close to the stratified value).
\item \textbf{ZTF operational prior (illustrative)}: a rough estimate
of the alert-stream class composition after standard real/bogus
filtering, in which SN~Ia comprises 65\%.
\end{enumerate}

The post-calibration high-confidence count at $p \geq 0.8$ is essentially
invariant across all three priors (425, 425, and 426 weighted objects,
respectively), reflecting that calibration is a per-object transformation
that does not depend on the class prior. The pre-calibration count
varies modestly (20, 18, 13), yielding implied gain factors of
21.2$\times$, 23.3$\times$, and 32.5$\times$. The gain claim is thus
not only robust but conservatively stated: under realistic deployment
priors that contain a higher fraction of SN~Ia, the gain factor is
larger, not smaller. The qualitative result---that calibration converts
a regime where almost no objects pass operational thresholds into one
where hundreds of high-precision candidates are available---is robust
across all three prior schemes.

We note the converse caveat for absolute precision values: at
$p \geq 0.8$, calibrated precision is 0.911 under the stratified prior
but 0.914 (natural BTS) and 0.942 (ZTF stream). Higher SN~Ia
prevalence in the deployment-time prior yields higher precision because
the dominant class is well-classified.

\begin{table*}[t]
\centering
\caption{Operational threshold counts and precision for ALeRCE under
three class-prior weighting schemes. Counts are sample-size-weighted
to reflect each target prior. The pre-calibration column shows that
almost no objects pass $p \geq 0.8$ regardless of prior, consistent
with ALeRCE's compressed confidence range. The post-calibration column
shows that the count of high-confidence objects is approximately
invariant across priors.
...
\caption{Sensitivity of ALeRCE post-calibration threshold counts to
assumed deployment-time class prior. Bars show the number of objects
passing five threshold values under three weighting schemes. The
post-calibration counts are approximately invariant across priors,
indicating that the operational gain claim does not hinge on the
stratified evaluation sample composition. \label{fig:prior_sensitivity}}
\end{figure}
...
We have presented, to our knowledge, the first systematic calibration audit of production machine learning classifiers deployed on ZTF transient alert streams. Our key findings are:
...
\begin{enumerate}
    ...
    \item Fink's two classifiers exhibit fundamentally different behavior on BTS objects. The Random Forest, designed for early-time alert classification, abstains on 93.9\% of BTS objects in a near-uniform pattern across classes---operational regime mismatch rather than miscalibration. SuperNNova exhibits implicit selective classification with class-dependent abstention (non-Ia transients abstain at 2.6$\times$ the SN~Ia rate, $\chi^2$ $p < 10^{-36}$), emitting non-zero scores for 64.1\% of objects with conditional ECE 0.183. Temperature scaling reduces conditional ECE to 0.051 ($T = 3.65$) but eliminates all $p \ge 0.8$ predictions, producing operationally null calibration at high thresholds. Pre-calibration SuperNNova scores remain useful for high-precision triage (209 objects at $p \ge 0.8$ with 73\% precision) despite formal miscalibration.
...
We caution that these findings characterize the calibration landscape of
current ZTF-era broker classifiers as deployed in early 2026; their
transferability to the Rubin/LSST era is bounded by differences in alert
cadence, photometric depth, broker pipeline evolution, and spectroscopic
ground-truth availability (Section~\ref{sec:lsst_implications}). The
methodological framework of coverage, conditional calibration, abstention
bias, and prior-aware operational metrics presented here is intended to
remain applicable as broker classifiers evolve. Future calibration audits
on Rubin alert packets will be required to verify the absolute calibration
values; the present audit aims to inform rather than forecast that work.
