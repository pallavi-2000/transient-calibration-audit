...
We performed two robustness checks to verify that our primary findings are not artifacts of data processing choices.

\textbf{ALeRCE renormalization.} Script 05 confirmed that restricting ALeRCE's 15-class output to the 4-class transient subset does not drive the observed miscalibration: renormalized ECE (0.271) is within 5\% of the raw 15-class ECE (0.283), with negligible per-class inflation factors (1.02--1.06$\times$).

\textbf{NEEDLE object-level deduplication.} The NEEDLE evaluation set contains 429 predictions from five held-out test sets across 278 unique objects. Of these, 43 objects appear in multiple test sets (mean 1.54 models per object overall; rare-class objects appear in up to all 5 sets). We deduplicate to the object level by averaging model probabilities per unique ZTF identifier and renormalizing. The object-level ECE (0.048) is lower than the model-instance ECE (0.075), a 36\% reduction, because rare-class objects that are more miscalibrated contribute proportionally less weight when counted once. The 100\% inter-model class agreement across all 43 multi-model objects (zero disagreements) validates the averaging methodology: model variance in predicted class is zero, and confidence uncertainty dominates. The class-asymmetric miscalibration pattern (SN underconfident, SLSN-I and TDE overconfident) persists in both analyses, confirming that the finding is robust to evaluation-set construction choices. Object-level results are primary throughout this paper; model-instance results are reported as sensitivity analysis in \S\ref{sec:needle_sensitivity}.
...
Finally, even equal-mass ECE is a biased estimator of true calibration error \citep{roelofs2022}. We report bootstrap confidence intervals throughout to quantify estimation uncertainty, and we verified that our results are robust to the choice of bin count (ECE varies by $< 0.01$ for $M \in [10, 20]$).
