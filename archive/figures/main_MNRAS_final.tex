...
\title[Calibration Audit of ZTF Transient Classifiers]{Probability Calibration of ZTF-era Transient Classifiers for Spectroscopic Triage}

\author[P. Sati]{Pallavi Sati$^{1}$\thanks{E-mail: pallavisati23@gmail.com}
\newauthor
...
\begin{keywords}
methods: statistical -- methods: data analysis -- surveys -- supernovae: general -- transients: general
\end{keywords}
...
\begin{table}
\centering
\caption{Stratified Evaluation Sample Composition
\label{tab:sample}}
% tablehead - convert to tabular manually & \colhead{Count} & \colhead{Fraction}
}
...
\begin{table}
\centering
\caption{Calibration Audit Summary \label{tab:summary}}
% tablehead - convert to tabular manually & \colhead{Architecture} &
...
\hline
\end{tabular}
\begin{minipage}{\columnwidth}\vspace{2pt}\textit{Note.} \1\end{minipage}. NEEDLE object-level results are
...
\begin{table}
\centering
\caption{NEEDLE Per-Class Calibration (Object-Level, $N=278$) \label{tab:needle_perclass}}
% tablehead - convert to tabular manually & \colhead{$n_\mathrm{true}$} & \colhead{$n_\mathrm{pred}$} & \colhead{Accuracy} & \colhead{Mean Conf.} & \colhead{Gap} & \colhead{Direction}
}
\begin{tabular}{l c c}\hline\hline
...
\hline
\end{tabular}
\begin{minipage}{\columnwidth}\vspace{2pt}\textit{Note.} \1\end{minipage}$ exceeds $n_\mathrm{true}$ for SLSN-I and TDE because many non-rare objects are misclassified into those categories.}
\end{table}
...
\begin{table}
\centering
\caption{NEEDLE Object-Level Silent Failures ($\geq$90\% confidence, wrong) \label{tab:needle_silent}}
% tablehead - convert to tabular manually & \colhead{Predicted} & \colhead{True} & \colhead{Confidence}
}
\begin{tabular}{l c c}\hline\hline
...
\hline
\end{tabular}
\begin{minipage}{\columnwidth}\vspace{2pt}\textit{Note.} \1\end{minipage}
\end{table}
